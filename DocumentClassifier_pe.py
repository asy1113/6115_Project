### H changed class FeatureInferencer(object) and class DocumentInferencer(object) in 20180413
    ...
    def process(self, annotations, relations):
        ...
        for relation_id, relation in relations.iterrows():
            ...
            if relation['arg1_cate'] != 'Modifier':
                ...
                continue
            if relation['arg2_cate'] == 'Modifier':          # added 2018                    
                continue                                     # added 2018
        ...
        for target_id, modifiers in sorted_modifiers.items():
            ...
            for rule_id, modifiers_in_rule in match_checker[source_type].items():
                ...
                #if modifiers_in_rule == modifiers or modifiers_in_rule == set(''):         # 2018 comment out
                
                if modifiers_in_rule.issubset(modifiers) or modifiers_in_rule == set(''):   # 2018 added  
                    matched_conclusion_types = list((inference_map[source_type][rule_id]))  
                    targets.remove(target_id)
                elif 'definite_negated_existence' in modifiers:                              # 2018 added
                    matched_conclusion_types.append(inference_map[source_type][rule_id])     # 2018 added
                    targets.remove(target_id)                                                # 2018 added
...
#                             
        #for target_id in targets:                                           # 2018 comment out # left over targets(pe) without modifiers
        #    type = annotations.loc[annotations['markup_id'] == target_id, 'type'].iloc[0]    
        #    matched_conclusion_types.append(type)                           
        #print("matched_conclusion_types")
        #print(matched_conclusion_types)
        #if 'negated_concept' in matched_conclusion_types:          ## 2018 added
        #    matched_conclusion_types = ['negated_concept']         ## 2018 added
...
class DocumentInferencer(object):
    ...
    def __init__(self, ruleFile, header_lines=0, delimiter=','):
        ...
        for rule in rules:
            ...
            # 2018 change if len(rules) == 1:      # 2018 change rules to rule
            if len(rule) == 1:    
        ...
        for rule_id, matcher in self.rule_matchers.items():     
            if matcher.issubset(matched_conclusion_types):       
                return self.doc_conclusions[rule_id]               
                          # doc_conclusion may be put into rule_matchers as part of dict() to match each other.
